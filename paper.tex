...
% Title and authors
\title{Equilibrium Propagation and Variational Quantum Circuits for Automated Detection of Acute Myeloid Leukemia from Blood Cell Microscopy Images}
...
\begin{abstract}
Quantum machine learning offers transformative potential for medical image analysis through quantum parallelism and entanglement-based feature encoding. We present comprehensive implementations of equilibrium propagation (EP), a biologically-inspired energy-based learning algorithm, and variational quantum circuits (VQCs) for automated detection of Acute Myeloid Leukemia (AML) from blood cell microscopy images. Using a clinical dataset of 18,365 manually annotated images from Munich University Hospital, we systematically evaluate both approaches against classical baselines across varying dataset sizes (50-250 samples per class). Our EP implementation achieves 86.4\% accuracy using a 256-128-64 architecture with 20 engineered statistical, textural, and morphological features, trained via energy minimization without backpropagation. The 4-qubit VQC employs ZZFeatureMap encoding with RealAmplitudes ansatz, achieving 83\% accuracy with robust performance across dataset scales. Comprehensive benchmarking reveals EP training completes in 89.4s with 0.13s inference, while VQC requires 180s training (simulation-limited) with 1.0s inference. Results demonstrate quantum-inspired and quantum approaches achieve clinically relevant accuracy while offering advantages for neuromorphic and NISQ-era quantum hardware deployment.
\end{abstract}
